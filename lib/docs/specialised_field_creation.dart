...
///
/// You can introduce your own specialized form field by defining a custom
/// `FormatterValidator` and exposing its default list via `FormatterValidatorDefaults`.
/// Your field then uses those defaults automatically (while still allowing the app
/// to add extra validators).
///
/// TL;DR steps
///
/// 1. Create a validator for your value type:
///    - Extend `FormatterValidator<I, V>` with explicit generics (e.g., `String` input → `PhoneNumber` value).
...
/// }
///
/// final class PhoneNumberFormatterValidator
///     extends FormatterValidator<String, PhoneNumber> {
///   @override
///   FieldContent<String, PhoneNumber> run(
///     BricksLocalizations l10n,
///     String keyString,
...
///   `addFormatterValidatorListMaker` (configurable order). By default, defaults run first, then additions.
/// - Chain policy: By default the chain is FullRun (runs all steps). You can switch to EarlyStop if desired
///   (stop after first valid result).
/// - Schema generator: The generator records the two makers on the descriptor. At runtime, `FormManager`
///   builds the chain from the descriptor—your defaults are picked up automatically.
class _DocsDummy{}
