...
- Also defines `focusedKeyString`, specifying which field should gain focus when the form first appears.

## 2. FormManager setup
- The form widget creates a `FormManager`, providing it with a `FormSchema` and a `FormData`.
- If `FormData` is meant to persist, it should be stored higher in the widget tree.
- Upon initialization, `FormManager` populates its `FormData.fieldDataMap`:
...
- Validation results (error messages) are stored in each `FormFieldData.fieldContent.error`.
...
- Updates both the input and validation result in the corresponding `FormFieldData`.
